...
export function createPgStorage(db: Db): IStorage {
  return {
    ...
    async getTripsByUserId(userId: string) {
      ...
      return allTrips.filter((t) => t.organizerId === userId || memberTripIds.has(t.id)) as Trip[];
    },
    ...
    async addTripMember(member: InsertTripMember) {
      const [row] = await db
        .insert(tripMembers)
        .values({ ...member, rsvpStatus: 'accepted' } as typeof tripMembers.$inferInsert)
        .returning();
    ...
    async getItineraryItems(tripId: string) {
      return db.select().from(itineraryItems).where(eq(itineraryItems.tripId, tripId)) as Promise<
        ItineraryItem[]
      >;
    },
    ...
    async getCommentsByTrip(tripId: string) {
      ...
      return db.select().from(comments).where(inArray(comments.itemId, itemIds)) as Promise<
        Comment[]
      >;
    },
    ...
    async createComment(comment: InsertComment) {
      const [row] = await db
        .insert(comments)
        .values(comment as typeof comments.$inferInsert)
        .returning();
    ...
    async createOrUpdateVote(vote: InsertVote) {
      ...
      const [row] = await db
        .insert(votes)
        .values(vote as typeof votes.$inferInsert)
        .returning();
    ...
    async createExpense(expense: InsertExpense) {
      const [row] = await db
        .insert(expenses)
        .values({ ...expense, isSettled: false } as typeof expenses.$inferInsert)
        .returning();
    ...
    async getInvitesByTrip(tripId: string) {
      return db.select().from(tripInvites).where(eq(tripInvites.tripId, tripId)) as Promise<
        TripInvite[]
      >;
    },
    ...
    async createInvite(invite: InsertTripInvite) {
      const [row] = await db
        .insert(tripInvites)
        .values(invite as typeof tripInvites.$inferInsert)
        .returning();
    ...
    async getPreferencesByTrip(tripId: string) {
      ...
      const userRows = await db.select().from(users).where(inArray(users.id, userIds));
      const userMap = new Map(userRows.map((u) => [u.id, u]));
      return prefs.map((p) => ({ ...p, user: userMap.get(p.userId)! })) as (MemberPreference & {
        user: User;
      })[];
    },
    ...
    async createOrUpdatePreference(pref: InsertMemberPreference) {
      const existing = await db
        ...
        .from(memberPreferences)
        .where(
          and(eq(memberPreferences.tripId, pref.tripId), eq(memberPreferences.userId, pref.userId))
        )
        .limit(1);
      const data = {
      ...
      const [row] = await db
        ...
        .values(data as typeof memberPreferences.$inferInsert)
        .returning();
    ...
    async getChatMessagesByTrip(tripId: string) {
      ...
      const userRows = await db.select().from(users).where(inArray(users.id, userIds));
      const userMap = new Map(userRows.map((u) => [u.id, u]));
      return messages.map((m) => ({ ...m, user: userMap.get(m.userId)! })) as (ChatMessage & {
        user: User;
      })[];
    },
    ...
    async createChatMessage(msg: InsertChatMessage) {
      const [row] = await db
        .insert(chatMessages)
        .values(msg as typeof chatMessages.$inferInsert)
        .returning();
    ...
    async getPhotosByTrip(tripId: string) {
      ...
      const userRows = await db.select().from(users).where(inArray(users.id, userIds));
      const userMap = new Map(userRows.map((u) => [u.id, u]));
      return photos.map((p) => ({ ...p, user: userMap.get(p.userId)! })) as (TripPhoto & {
        user: User;
      })[];
    },
    ...
    async createTripPhoto(photo: InsertTripPhoto) {
      const [row] = await db
        .insert(tripPhotos)
        .values(photo as typeof tripPhotos.$inferInsert)
        .returning();
    ...
    async getPollsByTrip(tripId: string) {
      ...
      const userRows = await db.select().from(users).where(inArray(users.id, userIds));
      const userMap = new Map(userRows.map((u) => [u.id, u]));
      return list.map((p) => ({ ...p, createdBy: userMap.get(p.createdByUserId)! })) as (Poll & {
        createdBy: User;
      })[];
    },
    ...
    async createPoll(poll: InsertPoll) {
      const [row] = await db
        .insert(polls)
        .values(poll as typeof polls.$inferInsert)
        .returning();
    ...
    async createOrUpdatePollVote(vote: InsertPollVote) {
      ...
      if (existing.length > 0) {
        const [row] = await db
          .update(pollVotes)
          .set({ optionIndex: vote.optionIndex })
      ...
      const [row] = await db
        .insert(pollVotes)
        .values(vote as typeof pollVotes.$inferInsert)
        .returning();
    ...
    async getPackingByTrip(tripId: string) {
      ...
      const userIds = Array.from(
        new Set(list.map((p) => p.assignedToUserId).filter(Boolean))
      ) as string[];
      ...
      return list.map((p) => ({
        ...p,
        assignedTo: p.assignedToUserId ? userMap.get(p.assignedToUserId) : undefined,
      })) as (PackingItem & { assignedTo?: User })[];
    },

    async createPackingItem(item: InsertPackingItem) {
      const [row] = await db
        .insert(packingItems)
        .values(item as typeof packingItems.$inferInsert)
        .returning();
    ...
    async getTransportationByTrip(tripId: string) {
      const list = await db
        .select()
        .from(transportationEntries)
        .where(eq(transportationEntries.tripId, tripId));
      if (list.length === 0) return [];
      const userIds = Array.from(
        new Set(list.map((t) => t.driverUserId).filter(Boolean))
      ) as string[];
      ...
      return list.map((t) => ({
        ...t,
        driver: t.driverUserId ? userMap.get(t.driverUserId) : undefined,
      })) as (TransportationEntry & { driver?: User })[];
    },

    async createTransportationEntry(entry: InsertTransportationEntry) {
      const [row] = await db
        .insert(transportationEntries)
        .values(entry as typeof transportationEntries.$inferInsert)
        .returning();
    ...
    async updateTransportationEntry(id: string, updates: Partial<TransportationEntry>) {
      const [row] = await db
        .update(transportationEntries)
        .set(updates)
        .where(eq(transportationEntries.id, id))
        .returning();
    ...
    async getGroupAvailabilityByTrip(tripId: string) {
      const list = await db
        .select()
        .from(groupAvailability)
        .where(eq(groupAvailability.tripId, tripId));
      if (list.length === 0) return [];
      const userIds = list.map((a) => a.userId);
      const userRows = await db.select().from(users).where(inArray(users.id, userIds));
      const userMap = new Map(userRows.map((u) => [u.id, u]));
      return list.map((a) => ({ ...a, user: userMap.get(a.userId)! })) as (GroupAvailability & {
        user: User;
      })[];
    },
    ...
    async setUserAvailability(tripId: string, userId: string, availableDates: string[]) {
      const existing = await db
        .select()
        .from(groupAvailability)
        .where(and(eq(groupAvailability.tripId, tripId), eq(groupAvailability.userId, userId)))
        .limit(1);
      if (existing.length > 0) {
        const [row] = await db
          .update(groupAvailability)
          .set({ availableDates })
          .where(eq(groupAvailability.id, existing[0].id))
          .returning();
        return row as GroupAvailability;
      }
      const id = crypto.randomUUID();
      const [row] = await db
        .insert(groupAvailability)
        .values({ id, tripId, userId, availableDates } as typeof groupAvailability.$inferInsert)
        .returning();
    ...
    async getDocumentsByTrip(tripId: string) {
      ...
      return list.map((d) => ({
        ...d,
        uploadedBy: userMap.get(d.uploadedByUserId),
      })) as (TripDocument & { uploadedBy?: User })[];
    },
    ...
    async createTripDocument(doc: InsertTripDocument) {
      ...
      const [row] = await db
        ...
        .values({ ...doc, id } as typeof tripDocuments.$inferInsert)
        .returning();
    ...
    async getEmergencyContactsByTrip(tripId: string) {
      return db
        ...
        .where(eq(emergencyContacts.tripId, tripId)) as Promise<EmergencyContact[]>;
    },
    ...
    async createEmergencyContact(contact: InsertEmergencyContact) {
      ...
      const [row] = await db
        ...
        .values({ ...contact, id } as typeof emergencyContacts.$inferInsert)
        .returning();
    ...
    async getMoodBoardByTrip(tripId: string) {
      ...
      return list.map((m) => ({
        ...m,
        addedBy: userMap.get(m.addedByUserId),
      })) as (MoodBoardItem & { addedBy?: User })[];
    },
    ...
    async createMoodBoardItem(item: InsertMoodBoardItem) {
      ...
      const [row] = await db
        ...
        .values({ ...item, id } as typeof moodBoardItems.$inferInsert)
        .returning();
    ...
    async getUserLearnedPreferences(userId: string) {
      const [row] = await db
        .select()
        .from(userLearnedPreferences)
        .where(eq(userLearnedPreferences.userId, userId))
        .limit(1);
    ...
    async createOrUpdateUserLearnedPreferences(pref: InsertUserLearnedPreferences) {
      const existing = await db
        .select()
        .from(userLearnedPreferences)
        .where(eq(userLearnedPreferences.userId, pref.userId))
        .limit(1);
      const id = existing[0]?.id ?? pref.id ?? crypto.randomUUID();
      const values = { ...pref, id, updatedAt: new Date() };
      if (existing.length > 0) {
        const [row] = await db
          .update(userLearnedPreferences)
          .set(values)
          .where(eq(userLearnedPreferences.id, existing[0].id))
          .returning();
        return row as UserLearnedPreferences;
      }
      const [row] = await db
        .insert(userLearnedPreferences)
        .values(values as typeof userLearnedPreferences.$inferInsert)
        .returning();
    ...
    async getSatisfactionByTrip(tripId: string) {
      const list = await db
        .select()
        .from(tripSatisfaction)
        .where(eq(tripSatisfaction.tripId, tripId));
      if (list.length === 0) return [];
      const userIds = list.map((s) => s.userId);
      const userRows = await db.select().from(users).where(inArray(users.id, userIds));
      const userMap = new Map(userRows.map((u) => [u.id, u]));
      return list.map((s) => ({ ...s, user: userMap.get(s.userId)! })) as (TripSatisfaction & {
        user: User;
      })[];
    },
    ...
    async createOrUpdateTripSatisfaction(entry: InsertTripSatisfaction) {
      const existing = await db
        ...
        .from(tripSatisfaction)
        .where(
          and(eq(tripSatisfaction.tripId, entry.tripId), eq(tripSatisfaction.userId, entry.userId))
        )
        .limit(1);
      ...
      if (existing.length > 0) {
        const [row] = await db
          .update(tripSatisfaction)
          .set({ score: entry.score, comment: entry.comment ?? null })
          .where(eq(tripSatisfaction.id, existing[0].id))
          .returning();
        return row as TripSatisfaction;
      }
      const [row] = await db
        .insert(tripSatisfaction)
        .values(values as typeof tripSatisfaction.$inferInsert)
        .returning();
    ...
    async getLocationSharingByTrip(tripId: string) {
      const list = await db
        ...
        .where(eq(locationSharing.tripId, tripId));
      if (list.length === 0) return [];
      const userIds = list.map((l) => l.userId);
      const userRows = await db.select().from(users).where(inArray(users.id, userIds));
      const userMap = new Map(userRows.map((u) => [u.id, u]));
      return list.map((l) => ({ ...l, user: userMap.get(l.userId)! })) as (LocationSharing & {
        user: User;
      })[];
    },
    ...
    async setUserLocation(tripId: string, userId: string, lat: string, lng: string) {
      const existing = await db
        .select()
        .from(locationSharing)
        .where(and(eq(locationSharing.tripId, tripId), eq(locationSharing.userId, userId)))
        .limit(1);
      ...
      if (existing.length > 0) {
        const [row] = await db
          ...
          .where(eq(locationSharing.id, existing[0].id))
          .returning();
        return row as LocationSharing;
      }
      const [row] = await db
        .insert(locationSharing)
        .values(values as typeof locationSharing.$inferInsert)
        .returning();
