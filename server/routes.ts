...
import {
  ...
  conversationalPlanningSuggestion,
  suggestBudgetOptimization,
  generateTripRecap,
  generatePackingList,
  parseEmailForItinerary,
  type AtlasRichContext,
} from './ai-service';
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.get('/api/public/trips/:shareCode', async (req: Request, res: Response) => {
    try {
      ...
      const [items] = await Promise.all([storage.getItineraryItems(trip.id)]);
      res.json({
        trip,
        items,
      });
  ...
  app.post('/api/contact', async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Failed to send message. Please try again or email us directly.' });
    }
  });
      ...
      try {
        ...
        res.json({
          ...
          message: user.passwordHash
            ? 'Logged in'
            : 'Account created. Default password: password123',
        });
      } catch (error) {
  ...
  app.get('/api/auth/me', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        subscriptionExpiresAt: (user as { subscriptionExpiresAt?: Date | null })
    ...
    async (req: Request, res: Response) => {
      try {
        if (!isStripeEnabled()) {
          return res
            .status(503)
            .json({ error: 'Payments are not configured', upgradeUrl: '/pricing' });
        }
        const userId = (req as any).userId;
        const user = await storage.getUserById(userId);
        if (!user) return res.status(404).json({ error: 'User not found' });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!user?.stripeCustomerId) {
          return res
            .status(400)
            .json({ error: 'No billing account. Subscribe first from the pricing page.' });
        }
  ...
  // Get invoice PDF
  app.get(
    '/api/stripe/invoices/:invoiceId/pdf',
    requireAuth,
    requireStripe,
    async (req: Request, res: Response) => {
      try {
  ...
  app.get('/api/user/usage-stats', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const activeTrips = userTrips.filter(
        (trip: any) => trip.status !== 'completed' && trip.status !== 'cancelled'
      ).length;
  ...
  app.get('/api/demo/trip', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      for (const poll of pollsList) {
        ...
        const counts = (poll.options as string[]).map(
          (_, i) => pv.filter((v) => v.optionIndex === i).length
        );
      ...
      const votes: Record<
        string,
        { up: number; down: number; abstain: number; userVote?: string }
      > = {};
      ...
      allVotes.forEach((v) => {
        ...
        voteDetails[v.itemId].push({
          userId: v.userId,
          voteType: v.voteType,
          userName: memberMap.get(v.userId) ?? 'Unknown',
        });
      });
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        const userId = (req as any).userId;
        const tripData = req.body as TripWizardData & {
          title?: string;
          tripType?: string;
          voteDeadline?: string;
        };
  ...
  app.post(
    '/api/trips/:id/regenerate-itinerary',
    requireAuth,
    requireAI,
    requireTripAccess,
    requirePlanner,
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        generateItinerary(tripId, tripData, memberInputs).catch((err) =>
          console.error('Regenerate itinerary error:', err)
        );
      ...
      try {
        ...
        if (
          ...
          return res
            .status(400)
            .json({
              error: 'dayNumber, type, time, name, description, location, pricePerPerson required',
            });
        }
  ...
  // Votes
  app.post(
    '/api/trips/:tripId/items/:itemId/vote',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/trips/:tripId/items/bulk-vote',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/trips/:tripId/items/bulk-delete',
    requireAuth,
    requireTripAccess,
    requirePlanner,
    async (req: Request, res: Response) => {
      try {
        ...
        // Delete each item
        await Promise.all(itemIds.map((itemId: string) => storage.deleteItineraryItem(itemId)));
  ...
  // Comments
  app.post(
    '/api/trips/:tripId/items/:itemId/comments',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.get(
    '/api/trips/:tripId/expenses/settlements',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.get(
    '/api/trips/:tripId/expenses/balance/:userId1/:userId2',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/trips/:tripId/expenses/:expenseId/settle',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/trips/:tripId/expenses/settle-all',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
      ...
      try {
        ...
        if (!receiptData) {
          return res.status(422).json({
            ...
            message:
              'The image may be unclear or not a valid receipt. Please try again with a clearer image.',
          });
        }
  ...
  app.post(
    '/api/trips/:tripId/budget-optimize',
    requireAuth,
    requireAI,
    requireTripAccess,
    aiGenerationRateLimiter,
    async (req: Request, res: Response) => {
      try {
        ...
        const expenses = await storage.getExpensesByTrip(tripId);
        const items = await storage.getItineraryItems(tripId);
        const itineraryEstimated = items.reduce(
    ...
    try {
      ...
      if (action === 'accept') {
        ...
        if (!userId) {
          return res
            .status(401)
            .json({ error: 'Log in to accept. Redirect to /login?invite=' + inviteId });
        }
        const user = await storage.getUser(userId);
        if (!user || user.email.toLowerCase() !== invite.email.toLowerCase()) {
          return res
            .status(403)
            .json({ error: 'You must log in with ' + invite.email + ' to accept' });
        }
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const { budgetBand, pace, diet, budgetFlexibility, mustDoActivities, accessibility } =
          req.body;
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!rsvpStatus || !['pending', 'accepted', 'declined'].includes(rsvpStatus))
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          ...
          return res.status(400).json({ error: 'question and options (array of 2+) required' });
  ...
  app.post(
    '/api/trips/:tripId/polls/:pollId/vote',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!dayNumber || !type || !description)
          return res.status(400).json({ error: 'dayNumber, type, description required' });
  ...
  app.patch(
    '/api/trips/:tripId/transportation/:entryId',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!userId || !Array.isArray(availableDates))
          return res.status(400).json({ error: 'availableDates (array) required' });
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!uploadedByUserId || !type || !name || !url)
          return res.status(400).json({ error: 'uploadedByUserId, type, name, url required' });
  ...
  // Emergency contacts
  app.get(
    '/api/trips/:tripId/emergency-contacts',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.patch(
    '/api/trips/:tripId/emergency-contacts/:contactId',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.delete(
    '/api/trips/:tripId/mood-board/:itemId',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/trips/:tripId/generate-recap',
    requireAuth,
    requireAI,
    requireTripAccess,
    aiGenerationRateLimiter,
  ...
  app.post(
    '/api/trips/:tripId/generate-packing-list',
    requireAuth,
    requireAI,
    requireTripAccess,
    aiGenerationRateLimiter,
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const trip = await storage.getTrip(tripId);
        const { suggestions } = await parseEmailForItinerary({
          emailText: emailText.slice(0, 10000),
          destination: trip?.destination,
        });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (trip) {
          ...
          const vibesPreferred = Array.from(
            new Set([...(existing?.vibesPreferred ?? []), ...vibes])
          );
          const tripTypesPreferred = tripType
            ? Array.from(new Set([...(existing?.tripTypesPreferred ?? []), tripType]))
            : (existing?.tripTypesPreferred ?? []);
          const learnedFromTripIds = Array.from(
            new Set([...(existing?.learnedFromTripIds ?? []), tripId])
          ).slice(-50);
  ...
  app.post(
    '/api/trips/:tripId/push/subscribe',
    requireAuth,
    requirePush,
    requireTripAccess,
    async (req: Request, res: Response) => {
  ...
  app.post(
    '/api/trips/:tripId/suggest-resolution',
    requireAuth,
    requireAI,
    requireTripAccess,
    aiGenerationRateLimiter,
    async (req: Request, res: Response) => {
      try {
        ...
        if (!question || !Array.isArray(options) || !Array.isArray(voteCounts))
          return res.status(400).json({ error: 'question, options, voteCounts required' });
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const confirmedMembers = membersWithUsers.filter(
          (m) => (m as any).rsvpStatus !== 'declined'
        ).length;
        ...
        const tripDays =
          Number.isNaN(tripStart.getTime()) || Number.isNaN(tripEnd.getTime())
            ? null
            : Math.max(
                1,
                Math.round((tripEnd.getTime() - tripStart.getTime()) / (1000 * 60 * 60 * 24)) + 1
              );
        const daysWithoutActivities =
          tripDays != null ? Math.max(tripDays - daysWithActivities, 0) : 0;
        ...
        if (overBudget && overAmount > 0) {
          detectedIssues.push(
            `Budget overrun of $${overAmount}. Total spend $${totalExpenses} vs budget $${totalBudget}.`
          );
        }
        if (tripDays != null && daysWithoutActivities > 0) {
          detectedIssues.push(
            `${daysWithoutActivities} of ${tripDays} days have no planned activities.`
          );
        }
  ...
  app.get(
    '/api/trips/:tripId/insurance-quote',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/trips/:tripId/flight-watch',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
        const tripId = getParam(req.params.tripId);
        const { flight } = req.body as {
          flight?: { from?: string; to?: string; date?: string; airline?: string };
        };
        if (!flight) {
        ...
        res.json({
          success: true,
          message: 'Flight watch recorded. Price alerts will be available in a future update.',
        });
      } catch (error) {
  ...
  app.get(
    '/api/trips/:tripId/atlas/notifications',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/atlas/notifications/:notificationId/read',
    requireAuth,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    '/api/atlas/notifications/:notificationId/dismiss',
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const notificationId = getParam(req.params.notificationId);
  ...
  app.post(
    '/api/atlas/notifications/:notificationId/snooze',
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const notificationId = getParam(req.params.notificationId);
  ...
  app.get('/api/users/:userId/insights', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      trips.forEach((t) => {
        (t.vibes || []).forEach((v: string) => {
          vibesCount[v] = (vibesCount[v] ?? 0) + 1;
        });
      ...
      (learned?.vibesPreferred ?? []).forEach((v: string) => {
        vibesCount[v] = (vibesCount[v] ?? 0) + 1;
      });
      (learned?.tripTypesPreferred ?? []).forEach((tt: string) => {
        tripTypes[tt] = (tripTypes[tt] ?? 0) + 1;
      });
      const topVibes = Object.entries(vibesCount)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 3)
        .map(([k]) => k);
      const topTypes = Object.entries(tripTypes)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 2)
        .map(([k]) => k);
      const insight =
        topVibes.length > 0 || topTypes.length > 0
  ...
  app.delete(
    '/api/trips/:tripId/items/:itemId/comments/:commentId',
    requireAuth,
    requireTripAccess,
    async (req: Request, res: Response) => {
      try {
