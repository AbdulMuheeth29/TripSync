...
export async function getEffectiveTier(
  userId: string
): Promise<{ tier: Tier; expiresAt: Date | null }> {
  const user = await storage.getUserById(userId);
  const tier = (user as { subscriptionTier?: string })?.subscriptionTier ?? 'free';
  const expiresAt =
    (user as { subscriptionExpiresAt?: Date | null })?.subscriptionExpiresAt ?? null;
...
export async function canCreateTrip(
  ...
  const active = trips.filter((t) => ['planning', 'booking', 'active'].includes(t.status));
  if (active.length >= FREE_ACTIVE_TRIPS_LIMIT) {
    return {
      allowed: false,
      reason: `Free plan limited to ${FREE_ACTIVE_TRIPS_LIMIT} active trips. Upgrade to Pro for unlimited.`,
    };
  }
  return { allowed: true };
}

export async function canAddMemberToTrip(
  tripId: string
): Promise<{ allowed: boolean; reason?: string }> {
  ...
  if (members.length >= FREE_MEMBERS_PER_TRIP_LIMIT) {
    return {
      allowed: false,
      reason: `Free plan limited to ${FREE_MEMBERS_PER_TRIP_LIMIT} members per trip. Upgrade to Pro for unlimited.`,
    };
  }
  return { allowed: true };
}

export async function canAddPhotoToTrip(
  tripId: string
): Promise<{ allowed: boolean; reason?: string }> {
  const trip = await storage.getTrip(tripId);
  if (!trip) return { allowed: false, reason: 'Trip not found' };
  ...
  if (photos.length >= FREE_PHOTOS_PER_TRIP_LIMIT) {
    return {
      allowed: false,
      reason: `Free plan limited to ${FREE_PHOTOS_PER_TRIP_LIMIT} photos per trip. Upgrade to Pro for unlimited.`,
    };
  }
  return { allowed: true };
}
...
export async function canUseAIGeneration(
  ...
  if (hasExistingAI) {
    return {
      allowed: false,
      reason:
        'Free plan includes 1 AI-generated itinerary per trip. Upgrade to Pro for unlimited AI generations.',
    };
  }
  return { allowed: true };
}
