 ...
 * Validate file type
 */
function validateFileType(file: Express.Multer.File, allowedTypes: string[]): void {
...
export function registerUploadRoutes(app: Express) {
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const result = await cloudStorage.uploadFile(file.buffer, file.originalname, {
          folder: 'photos',
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res.status(500).json({
          error: error instanceof Error ? error.message : 'Failed to upload confirmation',
          code: 'UPLOAD_FAILED',
        });
      }
    }
   ...
   */
  app.delete('/api/upload/:key', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!cloudStorage.isAvailable()) {
        return res.status(503).json({
          error: 'File upload not configured',
   ...
   */
  app.get('/api/upload/signed-url/:key', requireAuth, async (req: Request, res: Response) => {
    try {
      if (!cloudStorage.isAvailable()) {
        return res.status(503).json({
          error: 'File upload not configured',
