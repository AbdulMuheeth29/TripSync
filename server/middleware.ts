 ...
 */
export async function requireTripAccess(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const membership = members.find((m) => m.userId === userId && m.rsvpStatus === 'accepted');
 ...
 * Requires requireTripAccess to be called first
 */
export function requirePlanner(req: Request, res: Response, next: NextFunction) {
 ...
 * Requires requireTripAccess to be called first
 */
export function requireBeforeVoteDeadline(req: Request, res: Response, next: NextFunction) {
  const trip = (req as any).trip;
