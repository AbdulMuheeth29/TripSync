  ...
  try {
    ...
    await Promise.all([
      db.tripMember.create({
        data: { tripId: tokyoTrip.id, userId: users[0].id, role: "organizer", rsvpStatus: "accepted", joinedAt: hoursFromNow(-720) }
      }),
      db.tripMember.create({
        data: { tripId: tokyoTrip.id, userId: users[1].id, role: "member", rsvpStatus: "accepted", joinedAt: hoursFromNow(-700) }
      }),
      db.tripMember.create({
        data: { tripId: tokyoTrip.id, userId: users[2].id, role: "member", rsvpStatus: "accepted", joinedAt: hoursFromNow(-680) }
      }),
      db.tripMember.create({
        data: { tripId: tokyoTrip.id, userId: users[3].id, role: "member", rsvpStatus: "pending", joinedAt: hoursFromNow(-660) }
      })
    ]);
    ...
    await Promise.all([
      db.tripMember.create({
        data: { tripId: parisTrip.id, userId: users[1].id, role: "organizer", rsvpStatus: "accepted", joinedAt: hoursFromNow(-480) }
      }),
      db.tripMember.create({
        data: { tripId: parisTrip.id, userId: users[3].id, role: "member", rsvpStatus: "accepted", joinedAt: hoursFromNow(-470) }
      })
    ]);
    ...
    await Promise.all([
      db.tripMember.create({
        data: { tripId: barcelonaTrip.id, userId: users[0].id, role: "organizer", rsvpStatus: "accepted", joinedAt: hoursFromNow(-1200) }
      }),
      db.tripMember.create({
        data: { tripId: barcelonaTrip.id, userId: users[1].id, role: "member", rsvpStatus: "accepted", joinedAt: hoursFromNow(-1190) }
      }),
      db.tripMember.create({
        data: { tripId: barcelonaTrip.id, userId: users[2].id, role: "member", rsvpStatus: "accepted", joinedAt: hoursFromNow(-1180) }
      }),
      db.tripMember.create({
        data: { tripId: barcelonaTrip.id, userId: users[3].id, role: "member", rsvpStatus: "accepted", joinedAt: hoursFromNow(-1170) }
      }),
      db.tripMember.create({
        data: { tripId: barcelonaTrip.id, userId: users[4].id, role: "member", rsvpStatus: "accepted", joinedAt: hoursFromNow(-1160) }
      })
    ]);
    ...
    const polls = await Promise.all([
      ...
      db.poll.create({
        data: {
          ...
          options: ["Gonpachi (Kill Bill restaurant)", "Tsukiji Fish Market", "Robot Restaurant", "Traditional Kaiseki"],
    ...
    const availability = await Promise.all([
      db.groupAvailability.create({
        data: {
          ...
          userId: users[0].id,
          availableDates: [
            daysFromNow(30), daysFromNow(31), daysFromNow(32), daysFromNow(33),
            daysFromNow(34), daysFromNow(35), daysFromNow(36), daysFromNow(37)
          ]
        }
      }),
      db.groupAvailability.create({
      ...
      db.groupAvailability.create({
        data: {
          ...
          userId: users[3].id,
          availableDates: [
            daysFromNow(31), daysFromNow(32), daysFromNow(33), daysFromNow(34),
            daysFromNow(35), daysFromNow(36), daysFromNow(37)
          ]
        }
      })
    ]);
    ...
    const chatMessages = await Promise.all([
      db.chatMessage.create({
        data: {
          tripId: tokyoTrip.id,
          userId: users[0].id,
          content: "Hey everyone! So excited for this trip. Let's finalize the itinerary this week!"
        }
      }),
      db.chatMessage.create({
        data: {
      ...
      db.chatMessage.create({
        data: {
          tripId: tokyoTrip.id,
          userId: users[2].id,
          content: "Quick question - does anyone know if we need cash for the temples? Or do they take cards?"
        }
      }),
      db.chatMessage.create({
        data: {
    ...
    const satisfaction = await Promise.all([
      db.satisfactionRating.create({
        data: {
          ...
          userId: users[0].id,
          rating: 5,
          feedback: "Best trip ever! Everything was perfect. The group was amazing and Barcelona exceeded expectations."
        }
      }),
      db.satisfactionRating.create({
        data: {
          ...
          userId: users[1].id,
          rating: 5,
          feedback: "Loved every moment! The food, the beaches, the architecture. Can't wait for the next trip!"
        }
      }),
      db.satisfactionRating.create({
        data: {
