      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-center">
            You've {isAtLimit ? "reached" : "almost reached"} your {limitInfo.label.toLowerCase()} limit
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4">
          {/* Usage Display */}
          <Card className={`p-4 ${isAtLimit ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
            <div className="flex items-center justify-between mb-3">
              ...
              <Badge variant={isAtLimit ? "destructive" : "secondary"} className={!isAtLimit ? "bg-amber-100 text-amber-800" : ""}>
                {currentUsage} / {limit}
              </Badge>
            </div>
            ...
            <p className={`text-sm ${isAtLimit ? 'text-red-800' : 'text-amber-800'}`}>
              {isAtLimit ? (
                <>You've used all {limit} {limit === 1 ? limitInfo.unit : limitInfo.unit + 's'} on your {currentPlan} plan</>
              ) : (
          ...
          <Card className="p-4 bg-blue-50 border-blue-200">
            ...
            <p className="text-sm text-blue-800">
              {isAtLimit ? (
                <>
                  You cannot {
                    limitType === "trips" ? "create new trips" :
                    limitType === "ai_generations" ? "generate new AI itineraries" :
                    limitType === "members" ? "add more team members" :
                    "upload more files"
                  } on the {currentPlan} plan until you upgrade or remove existing items.
                </>
              ) : (
                <>
                  You have {limit - currentUsage} {limit - currentUsage === 1 ? limitInfo.unit : limitInfo.unit + 's'} remaining.
          ...
          {currentPlan === "Free" && (
            <Card className="p-3 bg-green-50 border-green-200">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-sm font-semibold text-green-900">Try {upgradePlan} Free for 14 Days</p>
              </div>
