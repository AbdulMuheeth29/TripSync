            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground italic">e.g., {vibe.examples}</p>
                </div>
              </div>
            </Card>
          );
      ...
      {selectedVibe && (
        <div className="rounded-lg border border-primary/50 bg-primary/5 p-4">
          <p className="text-sm text-primary">
            ✨ Great choice! We'll plan a{' '}
            {VIBES.find((v) => v.id === selectedVibe)?.name.toLowerCase()} trip for you.
          </p>
        </div>
      )}
