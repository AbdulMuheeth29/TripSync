...
function normalizeUser(data: Record<string, unknown>): User {
  ...
  const expiresAt =
    data.subscriptionExpiresAt != null
      ? typeof data.subscriptionExpiresAt === 'string'
        ? new Date(data.subscriptionExpiresAt)
        : (data.subscriptionExpiresAt as Date)
      : null;
  return {
    ...data,
    createdAt,
...
function isProTier(user: User | null): boolean {
  ...
  const expiresAt = (user as { subscriptionExpiresAt?: Date | string | null })
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const subscriptionTier: SubscriptionTier =
    (user as { subscriptionTier?: string } | null)?.subscriptionTier === 'teams'
      ? 'teams'
