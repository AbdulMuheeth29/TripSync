...
export const atlasMonitoringState = pgTable('atlas_monitoring_state', {
  ...
  budgetUsagePercentage: numeric('budget_usage_percentage', { precision: 5, scale: 2 }).default(
    '0'
  ),
...
export const insertTransportationEntrySchema = createInsertSchema(transportationEntries).omit({
  createdAt: true,
});
export const insertGroupAvailabilitySchema = createInsertSchema(groupAvailability).omit({
  createdAt: true,
});
...
export const insertUserLearnedPreferencesSchema = createInsertSchema(userLearnedPreferences).omit({
  updatedAt: true,
});
export const insertTripSatisfactionSchema = createInsertSchema(tripSatisfaction).omit({
  createdAt: true,
});
export const insertLocationSharingSchema = createInsertSchema(locationSharing).omit({
  updatedAt: true,
});
